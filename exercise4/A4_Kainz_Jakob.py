...
def fetch_records(retmax, term):
    # Identifier for the database
    Entrez.email = config.ENTRAZ_EMAIL
    ...
    if len(unique_organisms) <= 5:
        ...
        unique_records = fetch_records(retmax+20, term)

    return unique_records


def write_records(term):
    records = fetch_records(20, term)
...
def align_sequence():
    ...
    sequence_file_name = "Gene_Sequences.csv"
...
write_records(config.GENE_NAME_ALTERNATIVE_3)
align_sequence()
